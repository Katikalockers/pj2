...
public class BossHandler {
    ...
    private void updateBoss3Bullets(List<Bullet> bullets, boolean bossAlive,
                                    List<Explosion> explosions, Image explosionSprite, int groundY) {
        // Stage 3 walkable platforms
        final double[] PLATFORM_X = {50, 480, 900};
        final double[] PLATFORM_Y = {580, 580, 580};
        final double[] PLATFORM_WIDTH = {280, 280, 280};
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < PLATFORM_X.length; i++) {
                if (by >= PLATFORM_Y[i] && bx >= PLATFORM_X[i] && bx <= PLATFORM_X[i] + PLATFORM_WIDTH[i]) {
                    hitPlatform = true;
                    break;
                }
            }
